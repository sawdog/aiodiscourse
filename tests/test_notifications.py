...
@pytest.mark.asyncio
async def test_notifications(monkeypatch):
...
@pytest.mark.asyncio
async def test_notifications_undocumented(monkeypatch):
    async with AsyncDiscourseClient(
            "https://demo.discourse.org", "KEY", "admin",
    ) as client:
        async def fake_post(endpoint, json=None):
            return {"ok": True, "ns": "notifications"}

        monkeypatch.setattr(client, "post", fake_post)
        ns_obj = getattr(client, "notifications")
        for m in dir(ns_obj):
